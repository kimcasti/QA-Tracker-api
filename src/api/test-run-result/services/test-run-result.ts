...
export type TestRunResultPayload = {
...
export type BatchTestRunResultSyncItem = {
...
export type BatchTestRunResultSyncPayload = {
  testRun?: unknown;
  project?: unknown;
  organization?: unknown;
...
export function extractRelationDocumentId(rawValue: unknown): string | null {
...
export async function resolveFunctionalityDocumentId(
...
export async function resolveTestCaseDocumentId(
  rawTestCase: unknown,
  projectDocumentId: string,
  fallbackDocumentId?: string | null,
) {
...
export async function resolveBugDocumentId(
  rawBug: unknown,
  projectDocumentId: string,
  fallbackDocumentId?: string | null,
) {
...
export function buildTestRunResultData(
...
export async function getSyncedTestRun(testRunDocumentId: string) {
...
export async function syncBatchTestRunResults(userId: number, payload: BatchTestRunResultSyncPayload) {
  ...
  const resolveFunctionalityFromCatalog = buildDocumentIdResolver(projectFunctionalities, [
    record => record.code,
  ]);
  ...
  const existingResultsByDocumentId = new Map(existingResults.map(item => [item.documentId, item]));
  ...
  return getSyncedTestRun(testRunDocumentId);
}

export default factories.createCoreService('api::test-run-result.test-run-result');
