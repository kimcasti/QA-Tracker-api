export const ACCESS_ROLE_CODES = ['owner', 'qa-lead', 'qa-engineer', 'manager', 'viewer'] as const;

export type AccessRoleCode = (typeof ACCESS_ROLE_CODES)[number];

export const READ_ROLES: AccessRoleCode[] = ['owner', 'qa-lead', 'qa-engineer', 'manager', 'viewer'];
export const MANAGE_ROLES: AccessRoleCode[] = ['owner', 'qa-lead', 'qa-engineer', 'manager'];
export const ENGINEERING_ROLES: AccessRoleCode[] = ['owner', 'qa-lead', 'qa-engineer'];
export const OWNER_ROLES: AccessRoleCode[] = ['owner'];
export const ADMIN_ROLES: AccessRoleCode[] = ['owner', 'qa-lead'];

export const ACCESS_ROLE_SEEDS: Array<{
  code: AccessRoleCode;
  name: string;
  description: string;
}> = [
  {
    code: 'owner',
    name: 'Owner',
    description: 'Organization owner with full administration access across the workspace.',
  },
  {
    code: 'qa-lead',
    name: 'QA Lead',
    description: 'Full tenant administration and QA management permissions.',
  },
  {
    code: 'qa-engineer',
    name: 'QA Engineer',
    description: 'Operational QA access for executions, cycles, bugs and test assets.',
  },
  {
    code: 'manager',
    name: 'Manager',
    description: 'Read access plus limited planning and tracking updates.',
  },
  {
    code: 'viewer',
    name: 'Viewer',
    description: 'Read-only access inside the organization workspace.',
  },
];

export const JIRA_ACTIONS = ['status', 'projects', 'types', 'configure', 'createIssue'].map(
  action => `api::jira-integration.jira-integration.${action}`,
);

export const EXPOSED_ACTIONS = [
  ...JIRA_ACTIONS,
  'api::participant-directory.participant-directory.members',
  'api::external-participant.external-participant.find',
  'api::external-participant.external-participant.findOne',
  'api::external-participant.external-participant.create',
  'api::external-participant.external-participant.update',
  'api::external-participant.external-participant.delete',
  'api::public-uat-session.public-uat-session.find',
  'api::public-uat-session.public-uat-session.findOne',
  'api::public-uat-session.public-uat-session.create',
  'api::public-uat-session.public-uat-session.update',
  'api::public-uat-session.public-uat-session.delete',
  'api::project.project.find',
  'api::project.project.findOne',
  'api::project.project.create',
  'api::project.project.update',
  'api::project.project.delete',
  'api::project.project.storyMap',
  'api::project.project.upsertStoryMap',
  'api::project-proposal.project-proposal.find',
  'api::project-proposal.project-proposal.findOne',
  'api::project-proposal.project-proposal.create',
  'api::project-proposal.project-proposal.update',
  'api::project-proposal.project-proposal.delete',
  'api::project-story-map.project-story-map.find',
  'api::project-story-map.project-story-map.findOne',
  'api::project-story-map.project-story-map.create',
  'api::project-story-map.project-story-map.update',
  'api::project-story-map.project-story-map.delete',
  'api::project-module.project-module.find',
  'api::project-module.project-module.findOne',
  'api::project-module.project-module.create',
  'api::project-module.project-module.update',
  'api::project-module.project-module.delete',
  'api::project-persona-role.project-persona-role.find',
  'api::project-persona-role.project-persona-role.findOne',
  'api::project-persona-role.project-persona-role.create',
  'api::project-persona-role.project-persona-role.update',
  'api::project-persona-role.project-persona-role.delete',
  'api::sprint.sprint.find',
  'api::sprint.sprint.findOne',
  'api::sprint.sprint.create',
  'api::sprint.sprint.update',
  'api::sprint.sprint.delete',
  'api::functionality.functionality.find',
  'api::functionality.functionality.findOne',
  'api::functionality.functionality.create',
  'api::functionality.functionality.update',
  'api::functionality.functionality.reorder',
  'api::functionality.functionality.delete',
  'api::test-case.test-case.find',
  'api::test-case.test-case.findOne',
  'api::test-case.test-case.create',
  'api::test-case.test-case.update',
  'api::test-case.test-case.reorder',
  'api::test-case.test-case.delete',
  'api::test-case-template.test-case-template.find',
  'api::test-case-template.test-case-template.findOne',
  'api::test-case-template.test-case-template.create',
  'api::test-case-template.test-case-template.update',
  'api::test-case-template.test-case-template.delete',
  'api::test-run.test-run.find',
  'api::test-run.test-run.findOne',
  'api::test-run.test-run.listSummary',
  'api::test-run.test-run.create',
  'api::test-run.test-run.update',
  'api::test-run.test-run.delete',
  'api::automation-ingestion.automation-ingestion.openRun',
  'api::automation-ingestion.automation-ingestion.publishResults',
  'api::automation-import-history.automation-import-history.find',
  'api::automation-import-history.automation-import-history.findOne',
  'api::automation-import-history.automation-import-history.create',
  'api::automation-import-history.automation-import-history.update',
  'api::automation-import-history.automation-import-history.delete',
  'api::test-plan.test-plan.find',
  'api::test-plan.test-plan.findOne',
  'api::test-plan.test-plan.create',
  'api::test-plan.test-plan.update',
  'api::test-plan.test-plan.delete',
  'api::test-run-result.test-run-result.find',
  'api::test-run-result.test-run-result.findOne',
  'api::test-run-result.test-run-result.batchSync',
  'api::test-run-result.test-run-result.create',
  'api::test-run-result.test-run-result.update',
  'api::test-run-result.test-run-result.delete',
  'api::test-cycle.test-cycle.find',
  'api::test-cycle.test-cycle.findOne',
  'api::test-cycle.test-cycle.listSummary',
  'api::test-cycle.test-cycle.create',
  'api::test-cycle.test-cycle.update',
  'api::test-cycle.test-cycle.delete',
  'api::test-cycle-execution.test-cycle-execution.find',
  'api::test-cycle-execution.test-cycle-execution.findOne',
  'api::test-cycle-execution.test-cycle-execution.batchSync',
  'api::test-cycle-execution.test-cycle-execution.create',
  'api::test-cycle-execution.test-cycle-execution.update',
  'api::test-cycle-execution.test-cycle-execution.persist',
  'api::test-cycle-execution.test-cycle-execution.delete',
  'api::bug.bug.find',
  'api::bug.bug.findOne',
  'api::bug.bug.listSummary',
  'api::bug.bug.create',
  'api::bug.bug.update',
  'api::bug.bug.delete',
  'api::meeting-note.meeting-note.find',
  'api::meeting-note.meeting-note.findOne',
  'api::meeting-note.meeting-note.create',
  'api::meeting-note.meeting-note.update',
  'api::meeting-note.meeting-note.delete',
  'api::personal-note.personal-note.find',
  'api::personal-note.personal-note.findOne',
  'api::personal-note.personal-note.create',
  'api::personal-note.personal-note.update',
  'api::personal-note.personal-note.delete',
  'api::project-comment.project-comment.find',
  'api::project-comment.project-comment.findOne',
  'api::project-comment.project-comment.create',
  'api::project-comment.project-comment.update',
  'api::project-comment.project-comment.delete',
  'api::me.me.projectContexts',
  'api::me.me.organizationUsage',
  'api::me.me.workspace',
  'api::me.me.updateOrganization',
  'api::ai.ai.generateTestCases',
  'api::ai.ai.improveMeetingNotes',
  'api::ai.ai.interpretExecutionEvidence',
  'api::ai.ai.recommendExecutionFunctionalities',
  'api::ai.ai.analyzeQaStrategyCandidates',
  'api::ai.ai.analyzeProject',
  'api::ai.ai.generateProjectWireframeBrief',
  'api::ai.ai.generateDeliveryUnitSummary',
  'api::ai.ai.analyzeTechnicalReport',
  'api::plan-access.plan-access.authorizeAi',
  'api::plan-access.plan-access.authorizeExport',
  'api::plan-access.plan-access.consumeExport',
  'api::plan-access.plan-access.authorizeReport',
  'api::organization-team.organization-team.current',
  'api::organization-team.organization-team.invite',
  'api::organization-team.organization-team.updateMemberRole',
  'api::organization-team.organization-team.updateMemberProjects',
  'api::organization-team.organization-team.deactivateMember',
  'api::organization-team.organization-team.reactivateMember',
  'api::organization-team.organization-team.resendInvitation',
  'api::organization-team.organization-team.cancelInvitation',
  'api::superadmin.superadmin.organizations',
  'api::superadmin.superadmin.billingRequests',
  'api::superadmin.superadmin.updateOrganization',
  'api::superadmin.superadmin.updateBillingRequest',
  'api::superadmin.superadmin.memberships',
  'api::superadmin.superadmin.invitations',
  'api::superadmin.superadmin.auditLogs',
  'api::superadmin.superadmin.invite',
  'api::superadmin.superadmin.resendInvitation',
  'api::superadmin.superadmin.cancelInvitation',
  'api::superadmin.superadmin.updateMembershipRole',
  'api::superadmin.superadmin.deactivateMembership',
  'api::superadmin.superadmin.reactivateMembership',
  'api::superadmin.superadmin.deleteMembership',
] as const;
